...
options.dateFormat=locale.dateFormat;};initEditor=function(){var doc,tabIndex;$sourceEditor=$('<textarea></textarea>').hide();$wysiwygEditor=$('<iframe frameborder="0"></iframe>');if(window.location.protocol==="https:")
$wysiwygEditor.attr("src","javascript:false");$editorContainer.append($wysiwygEditor).append($sourceEditor);wysiwygEditor=$wysiwygEditor[0];sourceEditor=$sourceEditor[0];base.width(options.width||$original.width());base.height(options.height||$original.height());doc=getWysiwygDoc();doc.open();doc.write(_tmpl("html",{charset:options.charset,style:options.style}));doc.close();$wysiwygDoc=$(doc);$wysiwygBody=$(doc.body);base.readOnly(!!options.readOnly);if($.sceditor.ie)
$wysiwygDoc.find("html").addClass("ie ie"+ $.sceditor.ie);if(/iPhone|iPod|iPad| wosbrowser\//i.test(navigator.userAgent)||$.sceditor.ie)
...
{if(focusEnd)
$wysiwygBody.append((elm=doc.createElement('div')));else
elm=body.firstChild;if(doc.createRange)
{rng=doc.createRange();rng.setStart(elm,0);rng.setEnd(elm,0);}
else
{rng=body.createTextRange();rng.moveToElementText(elm);rng.collapse(false);}
...
return $editorContainer.width();base.dimensions(width,null,saveWidth);return this;};base.dimensions=function(width,height,save){width=(!width&&width!==0)?false:width;height=(!height&&height!==0)?false:height;if(width===false&&height===false)
...
{if(save!==false)
options.width=width;if(width&&width.toString().indexOf('%'))
width=$editorContainer.width(width).width();$wysiwygEditor.width(width- $wysiwygEditor.data('outerWidthOffset'));$sourceEditor.width(width- $sourceEditor.data('outerWidthOffset'));if(height===false)
{height=$editorContainer.height();save=false;}}
if(height!==false)
{if(save!==false)
options.height=height;if(height&&height.toString().indexOf('%'))
{height=$editorContainer.height(height).height();$editorContainer.height('auto');}
height-=!options.toolbarContainer?$toolbar.outerHeight(true):0;$wysiwygEditor.height(height- $wysiwygEditor.data('outerHeightOffset'));$sourceEditor.height(height- $sourceEditor.data('outerHeightOffset'));}
...
base.height(height);};base.destroy=function(){pluginManager.destroy();rangeHelper=null;lastRange=null;pluginManager=null;$(document).unbind('click',handleDocumentClick);$(window).unbind('resize orientationChanged',handleWindowResize);$(original.form).unbind('reset',handleFormReset).unbind('submit',base.updateOriginal);$wysiwygBody.unbind();$wysiwygDoc.unbind().find('*').remove();$sourceEditor.unbind().remove();$editorContainer.unbind().find('*').unbind().remove();$editorContainer.remove();$original.removeData('sceditor').removeData('sceditorbbcode').show();if(isRequired)
$original.attr('required','required');};base.createDropDown=function(menuItem,dropDownName,content,ieUnselectable){var onlyclose=$dropdown&&$dropdown.is('.sceditor-'+ dropDownName);base.closeDropDown();if(onlyclose)return;if(ieUnselectable!==false)
{$(content).find(':not(input,textarea)').filter(function(){return this.nodeType===1;}).attr('unselectable','on');}
var css={top:menuItem.offset().top,left:menuItem.offset().left,marginTop:menuItem.outerHeight()};$.extend(css,options.dropDownCss);$dropdown=$('<div class="sceditor-dropdown sceditor-'+ dropDownName+'" />').css(css).append(content).appendTo($('body')).click(function(e){e.stopPropagation();});};handleDocumentClick=function(e){if(e.which!==3)
base.closeDropDown();};handlePasteEvt=function(e){var html,elm=$wysiwygBody[0],checkCount=0,pastearea=elm.ownerDocument.createElement('div'),prePasteContent=elm.ownerDocument.createDocumentFragment();if(options.disablePasting)
...
{pastearea.innerHTML=html;handlePasteData(elm,pastearea);e.stopPropagation();e.preventDefault();return false;}}
while(elm.firstChild)
prePasteContent.appendChild(elm.firstChild);function handlePaste(elm,pastearea){if(elm.childNodes.length>0)
{while(elm.firstChild)
...
++checkCount;setTimeout(function(){handlePaste(elm,pastearea);},20);}}
handlePaste(elm,pastearea);base.focus();return true;};handlePasteData=function(elm,pastearea){$.sceditor.dom.fixNesting(pastearea);var pasteddata=pastearea.innerHTML;if(pluginManager.hasHandler("toSource"))
...
pasteddata=pluginManager.callOnlyFirst("toWysiwyg",pasteddata,true);rangeHelper.restoreRange();rangeHelper.insertHTML(replaceEmoticons(pasteddata));};base.closeDropDown=function(focus){var uploadFrame,emoticonFrame;if($('.sceditor-tinypic').length===1)
uploadFrame=$('.sceditor-tinypic');else if($('.sceditor-servimg').length===1)
uploadFrame=$('.sceditor-servimg');if($('.sceditor-emoticon').length===1)
emoticonFrame=$('.sceditor-emoticon');if(uploadFrame){uploadFrame.css('display','none');}
if(emoticonFrame){emoticonFrame.css('display','none');}
if($dropdown){$dropdown.unbind().remove();$dropdown=null;}
if(focus===true)
...
return wysiwygEditor.document;return null;};base.wysiwygEditorInsertHtml=function(html,endHtml,overrideCodeBlocking){base.focus();if(!overrideCodeBlocking&&($(rangeHelper.parentNode()).is('code')||$(rangeHelper.parentNode()).parents('code').length!==0))
return;rangeHelper.insertHTML(html,endHtml);appendNewLine();};base.wysiwygEditorInsertText=function(text,endText){base.wysiwygEditorInsertHtml($.sceditor.escapeEntities(text),$.sceditor.escapeEntities(endText));};base.insertText=function(text,endText){if(base.inSourceMode())
...
start=replaceEmoticons(start);base.wysiwygEditorInsertHtml(start);}
return this;};base.getWysiwygEditorValue=function(filter){var html;if($.sceditor.ie)
base.focus();rangeHelper.saveRange();$.sceditor.dom.fixNesting($wysiwygBody[0]);html=$wysiwygBody.html();if(filter!==false&&pluginManager.hasHandler("toSource"))
html=pluginManager.callOnlyFirst("toSource",html,$wysiwygBody);rangeHelper.restoreRange();lastRange=null;return html;};base.getBody=function(){return $wysiwygBody;};base.getContentAreaContainer=function(){return $wysiwygEditor;};base.getSourceEditorValue=function(filter){var val=$sourceEditor.val();if(filter!==false&&pluginManager.hasHandler("toWysiwyg"))
val=pluginManager.callOnlyFirst("toWysiwyg",val);return val;};base.setWysiwygEditorValue=function(value){if(!value)
value='<p>'+($.sceditor.ie?'':'<br />')+'</p>';$wysiwygBody[0].innerHTML=replaceEmoticons(value);appendNewLine();};base.setSourceEditorValue=function(value){$sourceEditor.val(value);};base.updateOriginal=function(){$original.val(base.val());};replaceEmoticons=function(html){if(!options.emoticonsEnabled)
return html;var emoticons=$.extend({},options.emoticons.more,options.emoticons.dropdown,options.emoticons.hidden);$.each(emoticons,function(key,url){var reg=$.sceditor.regexEscape(key)+"(?=([^\\<\\>]*?<(?!/code)|[^\\<\\>]*?$))",group='';if(options.emoticonsCompat)
{reg="((>|^|\\s|\xA0|\u2002|\u2003|\u2009|&nbsp;))"+ reg+"(?=(\\s|$|<|\xA0|\u2002|\u2003|\u2009|&nbsp;))";group='$1';}
html=html.replace(new RegExp(reg,'gm'),group+ _tmpl('emoticon',{key:key,url:url.url||url,tooltip:url.tooltip||key}));});return html;};base.inSourceMode=function(){return $editorContainer.hasClass('sourceMode');};base.sourceMode=function(enable){if(typeof enable!=='boolean')
...
check();else
setTimeout(check,100);};checkNodeChanged=function(){var node=rangeHelper.parentNode();if(currentNode!==node)
{$editorContainer.trigger($.Event('nodechanged',{oldNode:currentNode,newNode:node}));currentNode=node;}};base.currentNode=function(){return currentNode;};updateActiveButtons=function(e){var state,stateHandler,firstBlock,$button,parent,doc=$wysiwygDoc[0],i=btnStateHandlers.length,inSourceMode=base.sourceMode();if(!base.sourceMode()&&!base.readOnly())
...
$toolbar.find('.sceditor-button').removeClass('active');};handleKeyPress=function(e){var $parentNode,i=keyPressFuncs.length;base.closeDropDown();$parentNode=$(rangeHelper.parentNode());if(e.which===13)
...
return;while(i--)
keyPressFuncs[i].call(base,e,wysiwygEditor,$sourceEditor);};appendNewLine=function(){var name,inBlock;$.sceditor.dom.rTraverse($wysiwygBody,function(node){name=node.nodeName.toLowerCase();if($.inArray(name,requireNewLineFix)>-1)
inBlock=true;if((node.nodeType===3&&!/^\s*$/.test(node.nodeValue))||node.nodeName.toLowerCase()==='br'||($.sceditor.ie&&!node.firstChild&&!$.sceditor.dom.isInline(node,false)))
{if(inBlock){$($wysiwygBody).append($('<div>'+(!$.sceditor.ie?'<br />':'')+'</div>\n'));}
return false;}});var lastNode=$wysiwygBody.children().last();var blockingNodes={blockquote:1,code:1};if(lastNode.length){var lastNodeName=lastNode[0].nodeName.toLowerCase();if(lastNodeName=='p'){var lastChild=lastNode.children().last();if(lastChild.length&&lastChild[0].nodeName.toLowerCase()in blockingNodes)
lastNode.append("<br />");}
else if(lastNodeName in blockingNodes){$wysiwygBody.append("<br />");}}};handleFormReset=function(){base.val($original.val());};handleMouseDown=function(){base.closeDropDown();lastRange=null;};handleWindowResize=function(){var height=options.height,width=options.width;if(!base.maximize())
{if(height&&height.toString().indexOf("%")>-1)
base.height(height);if(width&&width.toString().indexOf("%")>-1)
base.width(width);}
else
base.height('100%',false).width('100%',false);};base._=function(){var args=arguments;if(locale&&locale[args[0]])
...
{if(!$blurElm)
$blurElm=$('<input style="width:0;height:0;opacity:0;border:0;padding:0;filter:alpha(opacity=0)" type="text" />').appendTo($editorContainer);$blurElm.removeAttr("disabled").show().focus().blur().hide().attr("disabled","disabled");}
else
...
{rangeHelper.saveRange();$wysiwygBody.html(replaceEmoticons($wysiwygBody.html()));rangeHelper.restoreRange();}}
else
{$wysiwygBody.find('img[data-sceditor-emoticon]').replaceWith(function(){return $(this).data('sceditor-emoticon');});$wysiwygBody.unbind('keypress',emoticonsKeyPress);}
...
shortcutHandlers[shortcut]=cmd;return this;};base.removeShortcut=function(shortcut){delete shortcutHandlers[shortcut.toLowerCase()];return this;};init();};$.sceditor.ie=(function(){var undef,v=3,div=document.createElement('div'),all=div.getElementsByTagName('i');do{div.innerHTML='<!--[if gt IE '+(++v)+']><i></i><![endif]-->';}while(all[0]);if((document.all&&window.atob))
v=10;return v>4?v:undef;}());$.sceditor.isWysiwygSupported=(function(){var match,contentEditable=$('<div contenteditable="true">')[0].contentEditable,contentEditableSupported=typeof contentEditable!=='undefined'&&contentEditable!=='inherit',userAgent=navigator.userAgent;if(!contentEditableSupported)
return false;var isUnsupported=/Opera Mobi|Opera Mini/i.test(userAgent);if(/Android/i.test(userAgent))
...
isUnsupported=!/OS ([5-9]|([1-9][0-9]+))(_\d)+ like Mac OS X/i.test(userAgent);if(/fennec/i.test(userAgent))
isUnsupported=false;return!isUnsupported;}());$.sceditor.regexEscape=function(str){return str.replace(/[\$\?\[\]\.\*\(\)\|\\]/g,"\\$&").replace("<","&lt;").replace(">","&gt;");};$.sceditor.escapeEntities=function(str){if(!str)
...
editor.closeDropDown(true);e.preventDefault();});editor.createDropDown(caller,"insertlink",content);},tooltip:"Insert a link"},unlink:{exec:"unlink",tooltip:"Unlink"},quote:{forceNewLineAfter:['blockquote'],exec:function(caller,html,author){var before='<blockquote>',end='</blockquote>';if(html)
{author=(author?'<cite>'+ author+'</cite>':'');before=before+ author+ html+ end+'<br />';end=null;}
else if(this.getRangeHelper().selectedHtml()==="")
end='<br />'+ end;this.wysiwygEditorInsertHtml(before,end);},tooltip:"Insert a Quote"},emoticon:{exec:function(caller){var editor=this;var createContent=function(includeMore){var endSpace=(editor.opts.emoticonsCompat?' ':''),$content=$('<div />'),$line=$('<div />').appendTo($content),emoticons=$.extend({},editor.opts.emoticons.dropdown,includeMore?editor.opts.emoticons.more:{}),perLine=0;for(var prop in emoticons)
{if(emoticons.hasOwnProperty(prop))
perLine++;}
perLine=Math.sqrt(perLine);$.each(emoticons,function(code,emoticon){$line.append($('<img />').attr({src:emoticon.url||emoticon,alt:code,title:emoticon.tooltip||code}).click(function(){editor.insert($(this).attr('alt')+ endSpace).closeDropDown(true);return false;}));if($line.children().length>=perLine)
...
return isW3C?range.cloneRange():range.duplicate();};base.selectedRange=function(){var range,parent,sel=isW3C?win.getSelection():doc.selection;if(!sel)
return;if(sel.getRangeAt&&sel.rangeCount<=0)
{range=doc.createRange();range.setStart(doc.body,0);sel.addRange(range);}
range=isW3C?sel.getRangeAt(0):sel.createRange();if(range.parentElement&&(parent=range.parentElement()))
if(parent.ownerDocument!==doc)
return;return range;};base.selectedHtml=function(){var div,range=base.selectedRange();if(!range)
...
{if(isW3C)
return range.commonAncestorContainer;if(range.parentElement)
return range.parentElement();}};base.getFirstBlockParent=function(n){var func=function(node){if(!$.sceditor.dom.isInline(node))
return node;var p=node?node.parentNode:null;return p?func(p):null;};return func(n||base.parentNode());};base.insertNodeAt=function(start,node){var currentRange=base.selectedRange(),range=base.cloneSelected();if(!range)
return false;range.collapse(start);if(range.insertNode)
range.insertNode(node);else
range.pasteHTML(node.outerHTML);base.selectRange(currentRange);};_createMarker=function(id){base.removeMarker(id);var marker=doc.createElement("span");marker.id=id;marker.style.lineHeight="0";marker.style.display="none";marker.className="sceditor-selection sceditor-ignore";return marker;};base.insertMarkers=function(){base.insertNodeAt(true,_createMarker(startMarker));base.insertNodeAt(false,_createMarker(endMarker));};base.getMarker=function(id){return doc.getElementById(id);};base.removeMarker=function(id){var marker=base.getMarker(id);if(marker)
...
range.select();else
{win.getSelection().removeAllRanges();win.getSelection().addRange(range);}};base.restoreRange=function(){var marker,range=base.selectedRange(),start=base.getMarker(startMarker),end=base.getMarker(endMarker);if(!start||!end||!range)
...
return ret;};base.raplaceKeyword=function(rep,includeAfter,repSorted,longestKey,requireWhiteSpace,curChar){if(!repSorted)
rep.sort(function(a,b){return a.length- b.length;});var before,after,str,i,start,left,pat,lookStart,maxKeyLen=longestKey||rep[rep.length-1][0].length;before=after=str="";if(requireWhiteSpace)
{if(!isW3C)
return false;++maxKeyLen;}
before=base.getOuterText(true,maxKeyLen);if(includeAfter)
after=base.getOuterText(false,maxKeyLen);str=before+(curChar!=null?curChar:"")+ after;i=rep.length;while(i--)
{pat=new RegExp("(?:[\\s\xA0\u2002\u2003\u2009])"+ $.sceditor.regexEscape(rep[i][0])+"(?=[\\s\xA0\u2002\u2003\u2009])");lookStart=before.length- 1- rep[i][0].length;if(requireWhiteSpace)
--lookStart;lookStart=Math.max(0,lookStart);if((!requireWhiteSpace&&(start=str.indexOf(rep[i][0],lookStart))>-1)||(requireWhiteSpace&&(start=str.substr(lookStart).search(pat))>-1))
{if(requireWhiteSpace)
start+=lookStart+ 1;if(start>before.length||start+rep[i][0].length+(requireWhiteSpace?1:0)<before.length)
continue;left=before.length- start;base.selectOuterText(left,rep[i][0].length-left-(curChar!=null&&/^\S/.test(curChar)?1:0));base.insertHTML(rep[i][1]);return true;}}
...
{return rangeA.compareEndPoints('EndToEnd',rangeB)===0&&rangeA.compareEndPoints('StartToStart',rangeB)===0;}
...
return false;node=next;}}},rTraverse:function(node,func,innermostFirst,siblingsOnly){this.traverse(node,func,innermostFirst,siblingsOnly,true);},blockLevelList:"|body|hr|p|div|h1|h2|h3|h4|h5|h6|address|pre|form|table|tbody|thead|tfoot|th|tr|td|li|ol|ul|blockquote|center|marquee|button|",isInline:function(elm,includeCodeAsBlock){if(!elm||elm.nodeType!==1)
...
{next=getSibling(node);previous=getSibling(node,true);sibling=node;trimStart=false;if(isInline(node))
{while((sibling=getSibling(sibling,true)))
{while(sibling.lastChild)
sibling=sibling.lastChild;if(!isInline(sibling)||sibling.nodeType===3)
{trimStart=sibling.nodeType===3?/[\t\n\r]$/.test(sibling.nodeValue):true;break;}}}
...
return this;return ret.length===1?ret[0]:$(ret);};})(jQuery,window,document);