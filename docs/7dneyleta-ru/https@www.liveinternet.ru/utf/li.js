								...
								if (elements['email'].value == '' || elements['body'].value == '') {
									...
									new Ajax.Request('sendcontact2.php', {
									  ...
									  onComplete:function(transport){
									  	LiCi.Notify.show('true','Спасибо, сообщение отправлено!');
						...
						onComplete:function(transport){
							...
							if (json.length>0) {
								for(var i=0;i<json.length;i++) {
									result += '<li><a href="'+json[i]['url']+'"><img src="../https@i.li.ru/ma/'+json[i]['url']+'/.gif" /></a><a class="uname" style="background-image:url(../https@i.li.ru/images/'+json[i]['gender']+'2.gif);" href="'+json[i]['url']+'">'+json[i]['name']+'</a></li>';
								};
								// Show popup
				...
				_widget: function(days, id) {
					...
					input.datepicker({
					    ...
					    onSelect: function(dateText) {
					        ...
					        window.location.href = '../www.liveinternet.ru/showjournal.php@journalid='+id+'&jday='+explode[2]+'&jyear='+explode[0]+'&jmonth='+explode[1];
					    }
					});
                        ...
                        twttr.anywhere(function (T) {
                             /* -[ Create twit area ]- */
                                   if (!T.isConnected()) {
                                       ...
                                       area.innerHTML = '<div id="tbox" style="padding:10px;background:#EEE;border:#DDD 1px solid;border-width:1px 0 1px 0;"><div style="width:100px;float:left; margin-top:6px;">Вы <a href="../www.twitter.com/'+user.screenName+'">@'+user.screenName+'</a><img style="display:block; margin-top:12px;" src="'+user.profileImageUrl+'" /></div></div><div id="tcomm"></div>';
                                       /* -[ Insert comment area ]- */
                                            T("#tbox").tweetBox({
                                                  height: 100,
                                                  width: 690,
                              ...
                              /* -[ Previous posted twits ]- */
                                   ...
                                   comm.innerHTML = '<iframe src="../www.liveinternet.ru/ReActive/static/twitter/default.htm" frameborder="0" width="100%" height="auto" style="margin-top:10px;padding:10px;"></iframe>';
                    ...
                    /* -[ Create if undefined ]- */
                              ...
                              if (wrapperOld.length > 0) {
                                   ...
                                   /* -[ Load libs ]- */    
                                       ...
                                       // LiCi.require('../https@stg.odnoklassniki.ru/share/odkl_share.js');
                              }                                                 
                              ...
                              if (obj.type == 'Like') {
                                   ...
                                   /* -[ Load FB ]- */
                                        ...
                                        fb.innerHTML = '<iframe src="../www.facebook.com/plugins/like.php@href='+url+'&amp;layout=button_count&amp;show_faces=false&ampaction=like&amp;font=trebuchet+ms&amp;colorscheme=light&amp;" scrolling="no" frameborder="0" allowTransparency="true" style="height:20px;"></iframe>';
                                   /* -[ Load other ]- */
                                        setTimeout(function(){
          ...
          LI.agelock.popup = function() {
               ...
               if (!shadow) {
                    ...
                    /* -[ Wrapper ]- */    
                         ...
                         wrapper.style.left = (LiCi.pageWidth() - 950) / 2 + 'px';
                         parent.appendChild( wrapper );                   
                         wrapper.innerHTML = '<div class="'+this.clName+'_wrapper_inner"><p>Автор не рекомендует просмотр этой страницы посетителям, моложе 18 лет.<span><a href="#" onclick="LI.agelock.accept();return false;">Мне есть 18 лет, открыть страницу</a><a href="../www.liveinternet.ru/top">На главную</a></span></p></div>';                                  
               }                   
          };         
     /* -----------[ Check ]----------- */    
...
----------------------------------------------- */              
	...
	/* -----------[ Init ]----------- */
		LI.printmedia.init = function() {
			setCookie('ucss', 'print', expires, "default.htm");
			window.location.href = document.location.href;
		};
	/* -----------[ Print ]----------- */
		LI.printmedia.print = function() {
			if (LiCi.getCookie('ucss') == 'print') {
				document.write('<link rel="stylesheet" type="text/css" media="screen" href="../www.liveinternet.ru/ReActive/css/blog-print.css" />');
			}
			setCookie('ucss', 'normal', expires, "default.htm");
		};
	     ...
	     /* -----------[ Init ]----------- */
		  ...
		  /* -----------[ Insert ]----------- */
		     _insert: function() {
		     	// Check even or odd
			/* 	if (bbuserid%2 == 0) {
				 	LiCi.$(':gl_Top-plug').innerHTML = this._template('Glm', 'Читай, пиши, комментируй вместе с LiveInternet.<b>Дневники</b> Offline', '../www.liveinternet.ru/click;libanner240');				 					 	
			 	} else {
				 	LiCi.$(':gl_Top-plug').innerHTML = this._template('4k', 'У Google Chrome появилось отличное расширение для удобного чтения дневников LiveInternet! <a target="_blank" href="../www.liveinternet.ru/click;libanner800@https_3A_2F_2Fchrome.google.com_2Fwebstore_2Fdetail_2Fliveinternet_25D0_25B4_25D0_25BD_25D0_25B5_25D0_25B0DC6C31E95">Хотите попробовать?</a><i class="gl_Top-plug_logo"></i>', '../www.liveinternet.ru/click;libanner800');
			 	}
			 	// Position and events
		  ...
		  /* -----------[ Template ]----------- */
		  	 _template: function(classname, string, url) {return '<div class="gl_Top-plug_'+classname+'"><div class="gl_Top-plug"><div class="gl_Top-plugI" id=":gl_Top-plugI">'+string+'<a class="gl_Top-plug_button" target="_blank" href="'+url+'@https_3A_2F_2Fchrome.google.com_2Fwebstore_2Fdetail_2Fliveinternet_25D0_25B4_25D0_25BD_25D0_25B5_25D0_25B0DC6C31E95" title="Установить расширение">Установить</a><i class="gl_Top-plug_close" id=":gl_Top-plug_close"><a class="" href="#">Закрыть</a> <b>[x]</b></i></div></div></div>';}
     };
