...
const inlinePayloads = self.__archiveSearchData || (self.__archiveSearchData = Object.create(null));
...
function dataScriptUrl(filename) {
  return new URL(filename.replace(/\.json$/, ".data.js"), resolveSearchBaseUrl()).toString();
}
...
function loadJsonWithScript(filename) {
  ...
  if (!scriptCache.has(filename)) {
    const promise = new Promise((resolve, reject) => {
      ...
      script.src = dataScriptUrl(filename);
      script.onload = () => {
        if (Object.prototype.hasOwnProperty.call(inlinePayloads, filename)) {
...
async function fetchJson(filename) {
  return loadJsonWithScript(filename);
}
...
self.ArchiveSearchFile = {
  async init(baseUrl) {
    if (typeof baseUrl === "string" && baseUrl) {
      state.baseUrl = new URL("./", baseUrl).toString();
    }
    const manifest = await ensureManifest();
    await getDateEntries();
    return { type: "ready", site_title: manifest.site_title || "" };
  },
  ...
  async search(query, limit) {
    ...
    if (!normalizedQuery) {
      state.lastMatches = [];
      return { type: "results", mode: "search", total: 0, items: [], message: "Введите поисковый запрос.", took_ms: 0 };
    }
    const searchResult = await runSearch(normalizedQuery, Number(limit) || 50);
    return {
      type: "results",
      mode: "search",
  ...
  async page(offset, limit) {
    ...
    const items = await buildItems(safeOffset, safeLimit, []);
    return {
      type: "results",
      mode: "page",
      total: state.lastMatches.length,
